...

{
  flake.homeModules.scroll =
    ...
    let
      ...
      windowResizerScript = pkgs.writers.writeLua "scroll-resize-windows-script" { } ''
        local scroll = require("scroll")

        local function on_view_event(view, change, _)
          scroll.log("ipc_view change=" .. tostring(change))

          if change ~= "move" then return end
          ...
          local layout = scroll.workspace_get_layout_type(workspace)
          scroll.log("view moved to workspace with layout=" .. tostring(layout))

          if layout == "vertical" then
            scroll.command(container, "set_size h 1.0")
            scroll.command(container, "set_size v 0.5")
          else
            scroll.command(container, "set_size h 0.5")
            scroll.command(container, "set_size v 1.0")
          end
        end

        scroll.add_callback("ipc_view", on_view_event, nil)
      '';
      ...
      scrollConfig = ''
        ...
        #
            ...
            bindsym $mod+m exec scrollnag -t warning -m 'You pressed the exit shortcut. Do you really want to exit scroll? This will end your Wayland session.' -B 'Yes, exit scroll' 'scrollmsg exit' -w 500 -e center -y overlay
        ...
        #
            ...
            bindsym $mod+Ctrl+home move beginning nomode
            bindsym $mod+Ctrl+end move end nomode
        ...
        bindsym $mod+Shift+w mode "fit_size"
