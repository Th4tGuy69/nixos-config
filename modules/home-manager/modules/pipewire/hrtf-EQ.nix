...
{
  flake.homeModules.hrtf-EQ =
    { config, ... }:
    let
      hrtfPath = "${config.home.homeDirectory}/.config/pipewire/hrtf/dtshx.wav";
      EQPath = "${config.home.homeDirectory}/.config/pipewire/eq/Astral.txt";
    in
    {
      home.file.".config/pipewire/hrtf/dtshx.wav".source = ./dtshx.wav;
      home.file.".config/pipewire/eq/Astral.txt".source = ./Astral.txt;
